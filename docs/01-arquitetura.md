...
| "Consumir o gateway do WebiCheck por meio de um adaptador, sem acoplar" | Porta `AiGateway` + adaptador HTTP; o core não conhece provedor nenhum |
...
**ADR-01 — Baileys atrás de porta, versão fixada exata.**
Não existe hoje release estável da linha 7.x (`latest` = `7.0.0-rc14`); o último
estável é `6.7.24`, publicado em 2026-07-29 sob a dist-tag `legacy`. Fixaremos
`baileys` em **`6.7.24` exato** (sem `^`, sem `~`) e isolaremos tudo atrás de
`WhatsAppGateway`. Detalhes e o plano de migração para 7.x estão em §6.1.
