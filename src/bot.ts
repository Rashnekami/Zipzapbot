...
async function onMessage(sock:WASocket,m:WAMessage){if(!m.message||m.key.fromMe||!m.key.remoteJid)return;const jid=m.key.remoteJid;const group=jid.endsWith("@g.us");const text=body(m).trim();logger.info({jid,group,hasText:Boolean(text),command:text.startsWith(config.PREFIX)},"Mensagem recebida");if(group&&!config.GROUPS_ENABLED)return;try{if(text.startsWith(config.PREFIX))return await command(sock,m,text);if(group&&!mentioned(m,sock)&&!repliesToMe(m,sock))return;if(!group&&!repliesToMe(m,sock)&&!mentioned(m,sock))return;const clean=text.replace(/@\d+/g,"").trim();if(!clean)return;addHistory(jid,{role:"user",content:clean});const answer=await askAI([{role:"system",content:config.AI_SYSTEM_PROMPT},...getHistory(jid)]);addHistory(jid,{role:"assistant",content:answer});await reply(sock,m,answer)}catch(e){logger.error(e);await react(sock,m,"❌");await reply(sock,m,e instanceof Error?e.message:"Erro inesperado")}}
