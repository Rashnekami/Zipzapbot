...
async function command(sock:WASocket,m:WAMessage,text:string){const jid=m.key.remoteJid!;const [raw,...rest]=text.slice(config.PREFIX.length).trim().split(/\s+/);const cmd=raw?.toLowerCase(),arg=rest.join(" ");
 ...
 if(cmd==="ia"){if(!arg)return reply(sock,m,`Use: ${config.PREFIX}ia sua pergunta`);addHistory(jid,{role:"user",content:arg});const answer=await askAI([{role:"system",content:config.AI_SYSTEM_PROMPT},...getHistory(jid)]);addHistory(jid,{role:"assistant",content:answer});return reply(sock,m,answer)}
...
async function onMessage(sock:WASocket,m:WAMessage){if(!m.message||m.key.fromMe||!m.key.remoteJid)return;const jid=m.key.remoteJid;const group=jid.endsWith("@g.us");const text=body(m).trim();logger.info({jid,group,hasText:Boolean(text),command:text.startsWith(config.PREFIX)},"Mensagem recebida");if(group&&!config.GROUPS_ENABLED)return;try{if(text.startsWith(config.PREFIX))return await command(sock,m,text);if(group&&!mentioned(m,sock)&&!repliesToMe(m,sock))return;const clean=text.replace(/@\d+/g,"").trim();if(!clean)return;addHistory(jid,{role:"user",content:clean});const answer=await askAI([{role:"system",content:config.AI_SYSTEM_PROMPT},...getHistory(jid)]);addHistory(jid,{role:"assistant",content:answer});await reply(sock,m,answer)}catch(e){logger.error(e);await react(sock,m,"❌");await reply(sock,m,e instanceof Error?e.message:"Erro inesperado")}}
export async function startBot(){await mkdir(config.DATA_DIR,{recursive:true});const {state,saveCreds}=await useMultiFileAuthState(`${config.DATA_DIR}/auth`);const sock=makeWASocket({auth:state,logger:logger as any,printQRInTerminal:false,browser:[config.BOT_NAME,"Chrome","2.0.0"],syncFullHistory:false});sock.ev.on("creds.update",saveCreds);sock.ev.on("messages.upsert",({messages,type})=>{logger.info({type,count:messages.length},"Evento de mensagens");for(const m of messages)void onMessage(sock,m)});sock.ev.on("connection.update",u=>{if(u.qr){updatePanel({qr:u.qr,status:"aguardando leitura do QR"});if(Date.now()-lastQrPrintedAt>=45000){lastQrPrintedAt=Date.now();qrcodeTerminal.generate(u.qr,{small:true},terminal=>console.log(`\nESCANEIE ESTE QR NO WHATSAPP (ele expira):\n${terminal}`))}}if(u.connection==="open"){lastQrPrintedAt=0;updatePanel({qr:"",status:"conectado"});logger.info("WhatsApp autenticado e pronto para receber mensagens")}if(u.connection==="close"){const code=(u.lastDisconnect?.error as Boom)?.output?.statusCode;updatePanel({qr:"",status:"desconectado"});if(code!==DisconnectReason.loggedOut)setTimeout(()=>void startBot(),3000);else logger.error("Sessão desconectada; apague data/auth para gerar novo QR")}})}
